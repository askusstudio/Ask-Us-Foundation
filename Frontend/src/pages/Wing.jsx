...
const WingsDetail = () => {
  ...
  // Backend wing key formatting (e.g., "WOMEN_WING")
  ...
  useEffect(() => {
    ...
    const fetchWingData = async () => {
      try {
        ...
        setStats(statsRes.data || { totalDonors: 0, totalAmount: 0 });
      } catch (error) {
      ...
      <section className={`${wing.heroBg}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 py-20 md:py-28">
          <div className="max-w-4xl">
            ...
            <Link
              to={`/donate?wing=${wingKey}&campaignTitle=${encodeURIComponent(wing.title)}`}
